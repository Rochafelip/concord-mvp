...
/** A visual divider between two groups of items, e.g. friend actions vs. moderation actions. */
...
export function ContextMenu({ children, items, disabled = false }: ContextMenuProps) {
  const hasActionableItems = items.some((item) => !('separator' in item));
  ...
  return (
    <RadixContextMenu.Root>
      <RadixContextMenu.Trigger asChild disabled={disabled || !hasActionableItems}>
        {children}
      ...
      <RadixContextMenu.Portal>
        <RadixContextMenu.Content className="z-[110] min-w-44 rounded border border-border bg-surface p-1 shadow-lg">
          {items.map((item, index) =>
            'separator' in item ? (
              <RadixContextMenu.Separator key={index} className="my-1 h-px bg-border" />
            ) : (
